...
### A-13: Validação visual de popup `GtkComboBoxText` automática indisponível em Mutter/GNOME 42

Local: `xdotool` + `import` + qualquer Skill `validacao-visual` que dependa de XTEST mouse events.
Risco: sprint que toca CSS/UI de `combobox` precisa proof-of-work de popup ABERTO. Mutter (window manager do GNOME 42 em Wayland-lookalike X11) **descarta XTEST `XTestFakeButtonEvent`** quando alvo é tab de `GtkNotebook` ou popup override-redirect de `GtkComboBox`. Sintomas reproduzidos em sessão `BUG-GUI-COMBOBOX-POPUP-CONTRAST-01` (2026-04-27 noite):
  - `xdotool getmouselocation` retorna `window:<WID-Hefesto>` (cursor está sobre a janela alvo).
  - `xdotool click 1`, `xdotool click --window WID 1`, `xdotool mousedown 1 + mouseup 1` separados, com `--delay 100` — **nenhum** troca a aba.
  - `xdotool key Tab` **funciona** (foco move entre widgets, comprovado via destacado de "Salvar Perfil").
  - `xdotool key alt+Down` no widget focado **não abre** popup do combobox.
  - O comportamento NÃO é grab race: ocorre mesmo após `windowfocus --sync`, `wmctrl -i -a`, `windowraise`.

Política operacional para sprints visuais:
  1. Validar **estaticamente** o CSS via `Gtk.CssProvider.load_from_data` + assertion sem `parsing-error`.
  2. Capturar **estado fechado** de cada widget afetado (essa parte funciona via `import -window WID`).
  3. Para POPUPS abertos / NOTEBOOKS navegados, anotar no proof-of-work: "validação visual humana solicitada — automação bloqueada por Mutter (A-13)". Validador-sprint não pode REPROVAR por isso, deve registrar como ressalva e pedir confirmação manual ao mantenedor.
  4. Tentativa programática alternativa: criar mini-script Python que abre janela isolada + `combobox.popup()` direto via PyGObject API — funciona se a app sob teste expuser API DBus, ou se for criado um harness de teste à parte.

Workaround conhecido: nenhum 100%. `wlrctl`, `evemu-event`, `ydotool` (em sessão Wayland) também são potencialmente bloqueados pelo guard de input synthesis do Mutter. Em sessão SSH+Xvfb headless o XTEST funciona — mas a captura visual final exige fonts/themes do host.
...
- 2026-04-28T — round 2 de hardening pós-v3.0.0 (4 sprints fechadas em sessão única). PR #106 BUG-DEB-DEPS-VENV-BUNDLED-01: `scripts/build_deb.sh` cria venv pinado em `/opt/hefesto-dualsense4unix/venv/` durante o build, com `python3.10 -m venv --copies` + `pip install --no-cache-dir .`. Wrappers `/usr/bin/` apontam para o venv. PyGObject continua sendo `python3-gi` do apt — herdado via `.pth` shim que injeta `/usr/lib/python3/dist-packages` no `sys.path` do venv. `Depends:` enxuto. `.deb` 228K → 8.3MB. Validado em `docker run ubuntu:22.04` puro: `apt install` + `--help` + `version` + todos imports OK. PR #105 BUG-DEB-AUTOSTART-WANTEDBY-DEFAULT-01: hipótese inicial (".deb não copiava unit") falsificada empiricamente — fix do path estava em commit `848660c`. Causa real: `WantedBy=graphical-session.target` cria symlink em homedir mas depende do DE ativar o target (race com login). Fix: `WantedBy=default.target` em `assets/hefesto-dualsense4unix.service`. `PartOf=` removido (daemon não usa DISPLAY). `gui-hotplug.service` mantém `graphical-session.target` (esse SIM precisa). Validado: `enable` cria symlink em `default.target.wants/`, `daemon-reexec` preserva `is-enabled=enabled`. PR #104 BUG-GUI-COMBOBOX-POPUP-CONTRAST-01: popup interno de `GtkComboBoxText` é window override-redirect que não herda escopo `.hefesto-dualsense4unix-window`. Adicionadas regras `combobox window.popup`, `combobox window menuitem`, `combobox window treeview` + estados `:hover`/`:selected` em `theme.css`. Validação visual do popup ABERTO bloqueada — armadilha A-13 adicionada (Mutter/GNOME 42 descarta XTEST mouse events). PR #107 BUG-DEB-GLYPHS-PATH-RESOLVER-01 (achado colateral): `_resolver_dir_glyphs()` em `button_glyph.py` só checava `~/.local/share/` e dev fallback; `.deb` instala em `/usr/share/hefesto-dualsense4unix/assets/glyphs/`. Path adicionado à lista de candidatos (usuário > sistema > dev). 16 glyphs físicos da aba Status restaurados em qualquer instalação `.deb` sem `~/.local/share/` populado.
