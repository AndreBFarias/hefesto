...
**Variante inversa (AUDIT-FINDING-PROFILE-MIC-LED-RESET-01, 2026-04-23):** campo **ausente** em `*Config` **mas aplicado** pelo `apply_*` com default regride estado runtime.
Sintoma concreto: `apply_led_settings` chamava `controller.set_mic_led(settings.mic_led)` incondicional. `LedsConfig` não tinha campo `mic_led`, `_to_led_settings` deixava no default `False`. Cada `profile.switch` ou autoswitch apagava o LED do mic mesmo quando usuário o havia mutado via botão físico ou IPC. Duas forças opostas chocando: fonte runtime (botão/IPC) tenta manter; profile apply zera em todo switch.
Fix canônico (opção c deste achado): quando um campo de hardware é **estado runtime puro** (muda via botão físico ou IPC dedicado e nunca é persistido em perfil), o `apply_*` do perfil NÃO deve tocá-lo. Remover a chamada ao setter do controller no apply; marcar o campo correspondente em `*Settings` como no-op documentado (ou removê-lo). Sempre que planejador cogitar adicionar um setter em apply, validar se o valor tem fonte runtime competidora.
Regra de revisão para planejador-sprint: para cada `controller.set_*(settings.<campo>)` chamada em `apply_*`, confirmar que `<campo>` é **propagável pelo schema** (`_to_led_settings` popula) e que **não há fonte runtime competidora** (botão físico, IPC dedicado). Se há fonte runtime, aplicar no profile switch é regressão.
...
**RESOLVIDA para `rumble.policy` (override por perfil)** (FEAT-RUMBLE-PER-PROFILE-OVERRIDE-01, 2026-04-24): `RumbleConfig` ganhou `policy: RumblePolicy | None = None` + `policy_custom_mult`. Mapper desta sprint é o trio `_effective_mult(profile_override=...)` + `ProfileManager.get_active_rumble_config()` + `RumbleEngine.link(profile_manager=...)`. 3 consumidores de `_effective_mult` consultam override via `daemon._profile_manager` (set em `subsystems/ipc.py`). Override ativo chega ao hardware via `reassert_rumble` (5Hz) e `apply_rumble_policy` (IPC). Teste #11 `test_engine_compute_mult_le_override_do_profile_manager` comprova que `RumbleEngine._compute_mult` aplica override quando linkado com ProfileManager. Teste #17 `test_profiles_default_nao_definem_policy_override` é anti-regressão para os 9 JSONs default (todos com `policy=None`). Pendência registrada em A-13: engine não é instanciado em produção — override funciona via `reassert_rumble` (caminho ativo), não via engine.

**Variante "Literal inline em config vira alias nomeado ao ser compartilhado" (FEAT-RUMBLE-PER-PROFILE-OVERRIDE-01, 2026-04-24):** quando schema de perfil precisa reusar um `Literal[...]` que hoje está inline em `daemon/lifecycle.py::DaemonConfig`, extrair para módulo dedicado neutro (zero imports internos) antes de importar no schema. Evita ciclo `schema → lifecycle → (transitivamente) schema`. Padrão aplicado: `src/hefesto/core/rumble_policy.py` expõe `RumblePolicy = Literal[...]`; `profiles/schema.py` e `daemon/lifecycle.py` ambos importam desse módulo puro. Regra de revisão para planejador: quando spec propõe reuso de Literal entre schema e config do daemon, conferir explicitamente via `rg "from hefesto\.daemon" src/hefesto/profiles/` que schema não ganha dep nova em daemon; se ganhar, refatorar para módulo neutro.
...
### A-13: `RumbleEngine` instanciado mas não linkado em produção (wire-up parcial)
Local: `src/hefesto/core/rumble.py:135` (classe) — zero instanciações em `src/hefesto/daemon/`. Consumido hoje apenas em `ipc_handlers.py:222-224` (leitura opcional de `last_mult_applied` via `getattr`) e `ipc_rumble_policy.py:45` (fallback quando ausente).
Risco: features de rumble policy (FEAT-RUMBLE-POLICY-01, FEAT-RUMBLE-PER-PROFILE-OVERRIDE-01) adicionam API no engine (`link`, `_compute_mult`, `update_auto_state`) mas o engine nunca é instanciado no `Daemon` nem plugado no poll loop. Todos os caminhos que interessam em produção usam funções **standalone** (`reassert_rumble`, `apply_rumble_policy`) lendo `daemon.config` e `daemon._profile_manager` diretamente. O engine fica letra morta até alguém fazer o wire-up completo.
Detectado em FEAT-RUMBLE-PER-PROFILE-OVERRIDE-01 (2026-04-24): sprint adicionou `RumbleEngine.link(..., profile_manager=...)` e testou com mock, mas `rg "RumbleEngine\(|rumble_engine\.link\(" src/hefesto/daemon` retornou zero matches. Override de policy por perfil chega ao hardware **apenas** via `reassert_rumble` (caminho ativo a 5Hz). Se alguém futuramente confiar em `engine._compute_mult` para estado em IPC ou para throttle do poll loop, vai receber valores default neutros (1.0) porque o engine não foi linkado.
Fix canônico: sprint derivada `CHORE-RUMBLE-ENGINE-WIRE-UP-01` deve (1) instanciar `RumbleEngine` no `Daemon.__init__` (slot `_rumble_engine`), (2) chamar `engine.link(config, state_ref, profile_manager=manager)` após `IpcServer.start()` em `subsystems/ipc.py`, (3) rotear `controller.set_rumble()` do poll loop pelo engine (para aplicar throttle 50Hz uniforme em todos os callers), (4) adicionar teste `test_daemon_wires_rumble_engine_to_profile_manager` validando que `daemon._rumble_engine._profile_manager is daemon._profile_manager`.
Regra de revisão para planejador: toda sprint que adiciona método a classe X deve checar `rg "X\(" src/hefesto` e confirmar que classe é instanciada em produção. Se só aparece em testes/docstrings, a sprint está preparando contrato para refactor futuro e DEVE listar explicitamente a sprint derivada de wire-up no não-escopo.
...
### L-21-8: Crescimento vertical de aba em GtkNotebook por linha extra de botões

Origem: FEAT-TRIGGER-PRESETS-IMPORT-EXPORT-01 (V2.5 #8, MERGED 2026-04-24, PR #100). A sprint adicionou uma 2ª linha de botões (`Exportar.../Importar...`) abaixo da linha existente (`Aplicar/Desligar`) em cada coluna L2/R2 da aba Gatilhos. Resultado em runtime: `GtkNotebook` calcula altura preferencial pela maior child (a aba Gatilhos virou a maior) e propaga essa altura para todas as outras abas (Status, Lightbar, Rumble, Perfis, Daemon, Emulação, Mouse e Teclado, Firmware), inflando a janela inteira. Sintoma confirmado pelo dono em Pop!_OS 22 — janela mais alta que necessário em todas as abas. **Regra:** ao adicionar linha extra em layout de aba, o planejador-sprint DEVE listar como item de proof-of-work obrigatório a captura visual de **outra aba** (não só a tocada) para flagrar inflação cross-tab. Critério de aceite "validação visual" sozinho com captura única é insuficiente — precisa de pelo menos 2 abas. Fix canônico quando aparece: consolidar a linha nova com a anterior (4 botões em 1 row, com `homogeneous=True`) em vez de empilhar em 2 rows. Aplicação geral: qualquer container que delega altura/largura preferida para o filho mais alto/largo (GtkNotebook, GtkPaned, GtkBox com `homogeneous=True`) tem efeito cross-child que precisa ser caçado em validação visual com escopo wider-than-the-touched-tab.
...
- 2026-04-24T — modo VALIDATE — validação FEAT-RUMBLE-PER-PROFILE-OVERRIDE-01 (PR #99, squash `51dbe92`). APROVADO_COM_RESSALVAS. 13 critérios de aceite todos entregues. Pytest 1316 → 1332 passed (+24 vs meta +17). Mypy 112 → 113 files. Smoke USB+BT verdes. Ressalva única: `RumbleEngine` não é instanciado em produção (condição pré-existente à sprint; override chega ao hardware via `reassert_rumble`, caminho ativo a 5Hz). Armadilha A-13 adicionada (wire-up parcial de RumbleEngine). A-06 ganhou seção RESOLVIDA para `rumble.policy` + variante "Literal inline extraído para módulo neutro" documentada. Sprint derivada registrada: `CHORE-RUMBLE-ENGINE-WIRE-UP-01`. Ponto 1 do executor validado: alias `RumblePolicy` em 3 locais (não 4) é equivalente semanticamente porque `core/rumble.py` consome via `RumbleConfig.policy: RumblePolicy | None`, preservando tipo sem import nominal. Ponto 3 (fixtures `_profile_manager=None` explícito) validado como pattern correto contra MagicMock auto-instanciando sub-mock.
- 2026-04-24T — modo VALIDATE — validação FEAT-TRIGGER-PRESETS-IMPORT-EXPORT-01 (PR #100, squash `fdf8ff2`). APROVADO_COM_RESSALVAS. 13 critérios de aceite todos entregues. Pytest 1340 → 1353 passed (+13 unit + derivações de mocks GTK). Ruff clean, mypy 114 → 115 files. Smoke USB+BT reportado verde pelo executor. Anonimato OK. Validação visual herdada do executor (PNG `/tmp/hefesto_gui_gatilhos_20260424T190813.png` sha256 `7b03...86`) — recaptura HEAD não foi feita porque o tray do app está em loop de crash em Pop!_OS 22 (issue separada, fora do escopo). Ressalva única **autorizada pelo dono no prompt**: a 2-row buttonbar do glade fez `GtkNotebook` adotar a altura da maior child, inflando altura de todas as abas. Fix em **PR sucessora dedicada** (consolida 4 botões em 1 linha por coluna). Lição metodológica nova: **L-21-8** registrada abaixo. A-06 não disparada (zero campos novos em `*Config` consumidos por mapper). A-04 verificado: glifos preservados; `tui/widgets/__init__.py` usa `▮▯` block elements (não `●○◐`), o grep do user-prompt era esperado retornar só `status_actions.py` — não é regressão.
