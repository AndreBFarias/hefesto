...
| 81 | [RELEASE] **Release v2.2.2** — bump 2.2.1→2.2.2 em pyproject.toml + `__init__.py` fallback + README; CHANGELOG promovido de [Unreleased] para [2.2.2] 2026-04-24; tag v2.2.2 pushada (run 24866299294): 5/6 jobs success, `deb-install-smoke` FALHOU com `ModuleNotFoundError: structlog.typing` — **não é o modo do fix 79.1** (pydantic), é structlog antigo no Jammy. `github-release` SKIPPED. Bloqueado por sprint 82. | — | — | BLOCKED_BY_82 |
| 82 | [BUG] **BUG-DEB-SMOKE-STRUCTLOG-TYPING-02** — P0-urgent. Compat layer no `logging_config.py`: `try: from structlog.typing import Processor; except ImportError: from structlog.types import Processor`; `packaging/debian/control` declara `python3-structlog (>= 21.5)`; teste novo `test_logging_compat_import.py`; após MERGED, re-dispatch `gh workflow run release.yml --field tag=v2.2.2`. L-21-7 dispara pela 2ª vez no mesmo ciclo. | XS | opus | ready |
