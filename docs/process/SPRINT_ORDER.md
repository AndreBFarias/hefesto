...
| **FEAT-TRIGGER-PRESETS-IMPORT-EXPORT-01** | MERGED 2026-04-24 | `fdf8ff2` (PR #100) | Aba Gatilhos ganha 4 botões `trigger_<side>_preset_<export\|import>` (1 par por coluna L2/R2). Schema novo `TriggerPreset` em `profiles/trigger_preset_schema.py` (wrapper `schema_version` + `name` + `exported_at` + `TriggerConfig`). IO puro com atomic write em `profiles/trigger_preset_io.py`. Handlers no `TriggersActionsMixin` reusam `preset_to_factory_args` para shape aninhado e populam draft sem chamar IPC (invariante: import é não-destrutivo). Suite 1340 → 1353 (+13 unit, mais derivações de mocks GTK). Mypy +1 file (115). **Follow-up visual em PR sucessora dedicada**: a 2-row buttonbar inflou altura de todas as abas via GtkNotebook (sintoma confirmado pelo dono em Pop!_OS 22); fix consolida 4 botões em 1 linha única por coluna (testado em stash local). |
...
| ~~8~~ | ~~**FEAT-TRIGGER-PRESETS-IMPORT-EXPORT-01**~~ | — | **MERGED 2026-04-24** (ver "Wave V2.5 — em execução" acima). Follow-up visual em PR sucessora dedicada (consolidação 4 botões em 1 linha por coluna). |
