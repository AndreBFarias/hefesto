...
## Última atualização: 2026-04-24 (Wave V2.5 — FEAT-TRIGGER-PRESETS-IMPORT-EXPORT-01 MERGED)

## Onde paramos

-2. **FEAT-TRIGGER-PRESETS-IMPORT-EXPORT-01 MERGED** 2026-04-24 via PR #100 (squash `fdf8ff2`). Aba Gatilhos ganha 4 botões `trigger_<side>_preset_<export|import>` (1 par por coluna L2/R2). Schema novo `TriggerPreset` (wrapper `schema_version` + `name` + `exported_at` + `TriggerConfig`) em `profiles/trigger_preset_schema.py`. IO puro com atomic write em `profiles/trigger_preset_io.py`. Handlers no `TriggersActionsMixin` reusam `preset_to_factory_args` para shape aninhado e populam draft sem chamar IPC (invariante: import é não-destrutivo). Pytest 1340 → 1353 passed / 8 skipped. Mypy 114 → 115 files. **Follow-up visual em PR sucessora dedicada**: a 2-row buttonbar inflou altura de todas as abas via `GtkNotebook` (sintoma confirmado pelo dono em Pop!_OS 22); fix consolida 4 botões em 1 linha por coluna (testado em stash local, fora desta sprint).
